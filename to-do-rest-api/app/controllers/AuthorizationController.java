...
public class AuthorizationController {
    ...
    @Inject
    public AuthorizationController(HttpExecutionContext ec, UsersStore userStore, UserSessionsStore userSessionsStore) {
    ...
    public Result refreshAccessToken(Http.Request request) {
        ...
        if (user == null) {
            return Results.forbidden("Invalid refresh token");
        }

        if (session == null) {
            return Results.forbidden("Invalid refresh token")
                    .withCookies(Http.Cookie.builder("refreshToken", null)
                            .withMaxAge(java.time.Duration.ZERO)
                            .build());
        }
